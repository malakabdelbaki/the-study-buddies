  ...
  <div className="p-8 max-w-2xl mx-auto">
    ...
    <div className="space-y-4">
      {quiz.questions?.length > 0 ? (
        quiz.questions.map((question: Question, questionIdx: number) => (
          <div key={`${question._id}-${questionIdx}`} className="border p-4 rounded">
            ...
            <ul className="space-y-2">
              {Object.keys(question.options).length > 0 ? (
                Object.keys(question.options).map((key: string, idx: number) => (
                  <li key={`${question._id}-${questionIdx}-${key}`}>
                    <button
